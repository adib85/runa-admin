...
/**
 * Sync Bronze Snake "Similar Products" → Shopify product metafield
 *
 * For each Bronze Snake product:
 *   1. Reads the cached "similar products" payload from DynamoDB
 *      (key: `<storeId>_similar_products_<handle>_<lang>`).
 *   2. Extracts the similar product Shopify IDs from `data.products[]`.
 *   3. Sets them on the source product's `runa.similar_products` metafield
 *      (type: list.product_reference) via the Shopify Admin GraphQL
 *      `metafieldsSet` mutation.
 *   4. Stamps `similar_metafield_synced_at` in Neo4j so we can resume.
 *
 * The PDP Liquid block reads this metafield as:
 *   {%- assign runa_similar = product.metafields.runa.similar_products.value -%}
 * so populating it lets the storefront render Similar Products without an
 ...
 *
 * Usage:
 *   node apps/api/src/scripts/sync-bronzesnake-similar-metafields.js [shop-domain] [options]
 *
 * Options:
 *   --dry-run             Don't write to Shopify or Neo4j, just report.
 *   --handle <handle>     Process a single product by handle.
 *   --missing             Only products that don't have similar_metafield_synced_at yet.
 ...
 *   --max-similar <n>     Cap how many similar IDs we publish per product (default: 12).
 *
 * Examples (read your access token from Lambda DB if not provided as $SHOP_TOKEN):
 *
 *   # Single product, DRY RUN — pure inspection, nothing written
 *   node apps/api/src/scripts/sync-bronzesnake-similar-metafields.js \
 *     bronze-snake-1.myshopify.com --handle some-handle --dry-run
 *
 *   # Single product, real write
 *   node apps/api/src/scripts/sync-bronzesnake-similar-metafields.js \
 *     bronze-snake-1.myshopify.com --handle some-handle
 *
 *   # All products missing the metafield, batch of 5 at a time
 *   node apps/api/src/scripts/sync-bronzesnake-similar-metafields.js \
...
const METAFIELD_KEY  = "similar_products";
...
const MAX_SIMILAR   = Math.max(1, parseInt(opt("max-similar", "12"), 10));
...
async function fetchProducts() {
  ...
  try {
    ...
    const missingFilter = MISSING_ONLY ? `AND p.similar_metafield_synced_at IS NULL` : "";
  ...
  try {
    ...
    await session.run(
      `MATCH (p:Product {id: $productId, storeId: $storeId})
       SET p.similar_metafield_synced_at = $syncedAt,
           p.similar_metafield_count     = $count
       RETURN p.id AS id`,
...
function buildSimilarCacheKey(handle, language = LANGUAGE) {
  return `${SHOP_DOMAIN.toLowerCase()}_similar_products_${String(handle).toLowerCase()}_${language}`;
}

async function readSimilarFromCache(handle) {
  const id = buildSimilarCacheKey(handle);
  const res = await dynamodb.get({ TableName: CACHE_TABLE, Key: { id } }).promise();
  if (!res.Item) return { cacheId: id, found: false, products: [] };
  const products = Array.isArray(res.Item?.data?.products) ? res.Item.data.products : [];
  return { cacheId: id, found: true, products };
}
...
async function setSimilarMetafield(productGid, gids, { maxRetries = 3 } = {}) {
  const input = [{
    ownerId: productGid,
...
function extractSimilarGids(cacheProducts, sourceProductId) {
  const seen = new Set();
  const out = [];
  for (const p of cacheProducts) {
    ...
    if (id == null) continue;
    if (String(id) === String(sourceProductId)) continue; // never include self
    const gid = toGid(id);
...
async function processProduct(product, idx, total, stats) {
  ...
  try {
    const cache = await readSimilarFromCache(product.handle);
    if (!cache.found) {
      console.log(`${tag}  SKIP — no cache (${cache.cacheId})`);
      stats.noCache++;
      return;
    }
    const rawGids = extractSimilarGids(cache.products, product.id);
    if (rawGids.length === 0) {
      console.log(`${tag}  SKIP — cache has no usable product ids`);
      stats.noIds++;
      return;
    }
    ...
    await setSimilarMetafield(productGid, gids);
...
async function main() {
  ...
  console.log(`\n═══════════════════════════════════════════════════════════`);
  console.log(`  Sync Similar-Products → Shopify metafield`);
  console.log(`  Shop:        ${SHOP_DOMAIN}`);
  console.log(`  Metafield:   ${METAFIELD_NS}.${METAFIELD_KEY} (${METAFIELD_TYPE})`);
  console.log(`  Mode:        ${DRY_RUN ? "DRY RUN (no writes)" : "LIVE"}`);
  console.log(`  Filter:      ${SINGLE_HANDLE ? `handle="${SINGLE_HANDLE}"` : MISSING_ONLY ? "MISSING similar_metafield_synced_at" : RECENT_ONLY ? `RECENT (last ${RECENT_HOURS}h)` : "ALL Bronze Snake products"}`);
  console.log(`  Cache lang:  ${LANGUAGE}`);
  console.log(`  Max ids:     ${MAX_SIMILAR} per product`);
